...
class SentimentAnalysis():
    ...
    def __init__(self):
        ...
        trainfile = basedir + 'input_data/processed/trainnews_sentimental_processed.csv'
        testfile = basedir + 'input_data/processed/testnews_sentimental_processed.csv'

        global dataTrain
        global accscore

        dataTrain = pd.read_csv(trainfile, sep=',', usecols = ['sentiment_score', 'headline_text', 'clean', 'vader_polarity'])
        dataTest = pd.read_csv(testfile, sep=',', usecols = ['sentiment_score', 'headline_text', 'clean', 'vader_polarity'])
        ...
        accscore = metrics.accuracy_score(dataTest['vader_polarity'], predicted_LogR)
        print("Sentiment Analysis Model Trained - accuracy:   %0.6f" % accscore)
